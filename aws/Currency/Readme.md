...
WORKDIR /src
COPY ["Currency.csproj", "Currency/"]
RUN dotnet restore "Currency/Currency.csproj"

WORKDIR "/src/Currency"
COPY . .
RUN dotnet build "Currency.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "Currency.csproj" \
...
Execute unit tests
```
    cd "Currency/test/Currency.Tests"
    dotnet test
```

Deploy application
```
    cd "Currency/src/Currency"
