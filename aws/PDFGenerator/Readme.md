...
COPY . .
RUN dotnet build "PDFGenerator.csproj" --configuration Release --output /app/build

FROM build AS publish
RUN dotnet publish "PDFGenerator.csproj" \
...
Execute unit tests
```
    cd "PDFGenerator/test/PDFGenerator.Tests"
    dotnet test
```

Deploy application
```
    cd "PDFGenerator/src/PDFGenerator"
